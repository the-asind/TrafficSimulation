...
public:
    ...
    void process_car(bool directionBool) { //0 - first, 1 - second TODO: init bool instead vector
        std::vector<Car> direction = second_direction_queue;
        if (!directionBool)
            std::vector<Car> direction = first_direction_queue;
...
int main() {
    ...
    if (check) {
        ...
        double sum_car_f, sum_car_s, sum_total_f, sum_total_s, sum_ratio_f, sum_ratio_s, sum_waiting_time;
        ...
        for (int i = 0; i < experiments_count; i++) {
            ...
            sum_car_f += simulation.car_f;
            sum_car_s += simulation.car_s;
            sum_total_f += simulation.total_f;
            sum_total_s += simulation.total_s;
            sum_waiting_time += avg_waiting_time;

            sum_ratio_f += (simulation.total_f/simulation.car_f);
            sum_ratio_s += (simulation.total_s/simulation.car_s);
        }

        std::cout << "\naverage cars in first direction:" << (sum_car_f/experiments_count) << "\naverage cars in second direction:" << (sum_car_s/experiments_count) << 
        "\naverage waiting time in first direction:" << (sum_ratio_f/experiments_count) <<  "\naverage waiting time in second direction:" << (sum_ratio_s/experiments_count) << 
        "\naverage waiting time:" << (sum_waiting_time/experiments_count) << std::endl;
    }
